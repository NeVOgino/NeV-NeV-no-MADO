...
#### 簡単な方法（項目・セクション管理機能を使用）

admin.htmlでは、JSONを直接編集せずに項目やセクションを簡単に管理できます：

**項目管理:**
- **追加**: 各セクションの「➕ 項目を追加」ボタンで新しい項目を追加
- **並び替え**: 各項目の↑↓ボタンで順序を変更
- **移動**: 「移動」ボタンで別のセクションへ移動
- **削除**: 「削除」ボタンで項目を削除

**セクション管理:**
- **追加**: 「➕ セクションを追加」ボタンで新しいセクションを作成
- **並び替え**: 各セクションの↑↓ボタンでセクションの順序を変更
- **削除**: 🗑️ボタンでセクションを削除

詳しくは **USER_GUIDE.md** の「簡単な編集機能（項目・セクション管理）」セクションを参照してください。
